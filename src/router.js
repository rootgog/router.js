export default class Router {
    ...
    for (var route in this.routes) {
      if (url == route) {
        ...
        if (url.match(regex)) {
          ...
          let wildcards = route.match(/(\:\w*)|\*/g);

          //get indexes
          wildcards.forEach((param) => {
            let index = route.split("/").indexOf(param);
            let name = param.substring(1);
            let value = url.split("/")[index];
            params[name] = value;
          });
  ...
  resolve({
    ...
    if (typeof route == "function") {
      ...
      route({ params: this.resolveparams });
      this.resolveparams = {};
    } else {
  ...
  sendTo({ path = window.location.pathname, url = undefined } = {}) {
    this.resolve({
      path: path,
      url: url,
    });
  ...
  setURL(url) {
    ...
    history.pushState(null, null, (base ? `/${base}` : "") + "/" + url);
    return this;
  }
}
